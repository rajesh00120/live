...
class AlbumsController extends Controller
{
    ...
    public function testSaveAlbum(Request $request)
    {   
        if(!$request->hasFile('media') || !isset($request->name) || !isset($request->place) || !isset($request->date))
            return response()->json([
                'status' => 0,
        ...
        $name = $request->name;
        $isAlbumAlreadyExist = Albums::where('name', '=', $name)->where('user_id', '=', $user_id)->first();
        $timeline_cover_image = Albums::where('user_id', '=', $user_id)->where('timeline_date',"$request->date")->where('timeline_cover','!=','null')->first();
        if($isAlbumAlreadyExist)
        return response()->json([
            'status' => 1,
            'message' => config('constant.ALBUM_ALREADY_EXIST')
        ...
        $newAlbums->name = $name;
        $newAlbums->date =  $request->date;
        $newAlbums->timeline_date =  $request->timeline_date;
        $newAlbums->place = $request->place;
        $newAlbums->description = isset($request->description)?$request->description:'';
        $newAlbums->timeline_cover = isset($timeline_cover_image->timeline_cover)?$timeline_cover_image->timeline_cover:'';
        $newAlbums->save();
        $album_id = $newAlbums->id;
    ...
    public function testTimelineAlbumList(Request $request)
    {
        //$user_id = $request->user->id;
        $userId = $request->user_id;
    ...
    {  
        ...
        if($follow_status==2 && ($userStatus->user_status==2)) {   //follow user status ACCEPT && userstatus private
            $userAlbums = Albums::select('*',DB::raw("YEAR(date) as Year"),DB::Raw('CONCAT("'.$img_path.'","/", timeline_cover) as timeline_cover'),DB::Raw('CONCAT("'.$img_path.'","/", album_cover) as album_cover'))->where('user_id', '=', $userId)->where('date',$date)->whereYear('date', '=', $Year)->whereMonth('date', '=', $Month)->orderBy('id','desc')->get()->toArray();
            if(!count($userAlbums))
                return response()->json([
                'status' => 0,
                'message' => config('constant.NO_ALBUM_FOUND')
    ...
    public function testUserAlbumsList(Request $request)
    {  
        ...
        if($follow_status==2 && ($userStatus->user_status==2)) {   //follow user status ACCEPT && userstatus private
            $userAlbums = Albums::select('*',DB::raw("YEAR(date) as Year"),DB::Raw('CONCAT("'.$img_path.'","/", timeline_cover) as timeline_cover'),DB::Raw('CONCAT("'.$img_path.'","/", album_cover) as album_cover'))->where('user_id', '=', $userId)->where('timeline_date',$date)->whereYear('timeline_date', '=', $Year)->whereMonth('timeline_date', '=', $Month)->orderBy('id','desc')->get()->toArray();
    ...
    public function getNewsDeatils($album_id){
        ...
        public function albumViewPostApi(Request $request){ //print_r($request->all()); die;
            ...
            // } 
            // print_r(isset($request->album_id));
            // print_r(isset($request->album_media_id));

            if(isset($request->album_id) && !isset($request->album_media_id)){
                // DB::connection()->enableQueryLog();
                $userAlbums_count = Album_views::where('album_id', '=', $request->album_id)->where('user_id','=',$request->user->id)->where('album_media_id', '=', null)->get()->count();
                if($userAlbums_count){
                    return response()->json([
                                    'status' => 1,
                                    'message' => 'You have already viewed this album.',
                                    'data'=>$userAlbums_count
                                ]);
                }
                else{
                    $arr = array (
                ...
                // print_r($userAlbums);
                // print_r(DB::getQueryLog($userAlbums));
            }
            else if(isset($request->album_id) && isset($request->album_media_id)){
                $userAlbums_count = Album_views::where('album_id', '=', $request->album_id)->where('album_media_id', '=', $request->album_media_id)->where('user_id','=',$request->user->id)->get()->count();
                if($userAlbums_count){
                    return response()->json([
                                    'status' => 1,
                                    'message' => 'You have already viewed this image.',
                                    'data'=>$userAlbums_count
                                ]);
                }
                else{
    ...
    public function updateTimeline(Request $request)
    {
        ...
        $date = $year.'-'.$month.'-'.$day;
        $getDetails = Albums::where('user_id', '=', $user_id)->where('timeline_date',$date)->whereYear('timeline_date', '=', $year)->whereMonth('timeline_date', '=', $month)->get()->toArray();
        ...
        if(!count($getDetails)){
            ...
            Albums::where('user_id', '=', $user_id)->where('timeline_date',$date)->whereYear('timeline_date', '=', $year)->whereMonth('timeline_date', '=', $month)->update($album_timeline);

            $get_up = Albums::where('user_id', '=', $user_id)->where('timeline_date',$date)->whereYear('timeline_date', '=', $year)->whereMonth('timeline_date', '=', $month)->first();
    ...
    public function updateAlbumCover(Request $request)
    {

        if(!$request->hasFile('album_cover') || !isset($request->album_id) ||!isset($request->album_name) || !isset($request->album_description))
        return response()->json([
            'status' => 0,
            'message' => config('constant.PARAMETER_MISSING')
        ]);
        ...
        $album_cover = $request->album_cover;
            ...
            //Move Uploaded File
            $file->move(public_path('album_images'),$filename);
            $album_timeline = array('name'=>$request->album_name,'description'=>$request->album_description, 'album_cover'=>$filename);
            Albums::where('user_id', '=', $request->user->id)->where('id',$request->album_id)->update($album_timeline);

            $get_up = Albums::where('user_id', '=', $request->user->id)->where('id',$request->album_id)->first();
